...
def test_loss_ratio_plan_uses_approved_supporting_joins(health_uk_catalogue) -> None:
    ...
    assert plan.result_shape.max_rows == 1
    assert "plans.plan_tier = plan_tier" in plan.filters


def test_incurred_claims_plan_uses_claim_header_path(health_uk_catalogue) -> None:
    request = AnalyticsRequest(
    ...
    plan = build_incurred_claims_plan(health_uk_catalogue, resolved)

    assert plan.tables == ("claims", "members", "plans")
    assert {(join.left_table, join.right_table) for join in plan.joins} == {
        ("claims", "members"),
        ("members", "plans"),
    }
    assert plan.result_shape.columns == ("incurred_claims",)
    assert plan.result_shape.max_rows == 1
    assert "claims.status != excluded_status" in plan.filters


def test_claim_severity_plan_uses_claim_line_path(health_uk_catalogue) -> None:
    request = AnalyticsRequest(
    ...
    plan = build_claim_severity_plan(health_uk_catalogue, resolved)
    ...
    assert plan.result_shape.columns == ("claim_severity",)
    assert plan.result_shape.max_rows == 1
    assert "claims.status = closed_status" in plan.filters
...
def test_decline_rate_plan_groups_by_consultant_specialty(health_uk_catalogue) -> None:
    ...
    assert plan.result_shape.max_rows == 50
